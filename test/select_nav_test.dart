...
void main() {
  ...
  testWidgets('StunningDivider renders', (WidgetTester tester) async {
    await tester.pumpWidget(_host(const SizedBox(width: 120, child: StunningDivider())));
    await tester.pumpAndSettle();
    expect(find.byType(StunningDivider), findsOneWidget);
  });

  testWidgets('StunningPagination advances on Next', (WidgetTester tester) async {
    var page = 0;

    await tester.pumpWidget(_host(StatefulBuilder(
      builder: (context, setState) => StunningPagination(
        pageCount: 5,
        currentPage: page,
